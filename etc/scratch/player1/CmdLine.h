    ...
    ParsedEntries ParseCommandLine( int numArgs, char** args ) const
    {
        ...
        while( i < numArgs )
        {
            ...
            if( IsLongParameter( arg ) )
            {
                ...
                if( ei == entries_.end() ) throw std::logic_error( "Error - unknown parameter " + arg );
            ...
            else if( IsShortParameter( arg ) )
            {
                ...
                if( si == synonyms_.end() ) throw std::logic_error( "Error - unknown parameter " + arg );
                Entries::const_iterator ei = entries_.find( si->second );
                if( ei == entries_.end() ) throw std::logic_error( "Error: cannot find synonym of short command " + si->first );
                if( ei->second.optional == false )
                {
                    mandatoryCommands.erase( ei->first );
                }
